...
def count_reads_in_features( sam_filename, gff_filename, samtype, order, stranded, 
      overlap_mode, feature_type, id_attribute, quiet, minaqual, samout, allow_ambiguous, allow_nonunique, use_NH ):
   ...
   try:
      if sam_filename != "-":
         ...
         read_seq = iter( SAM_or_BAM_Reader( sys.stdin ) )
         first_read = read_seq.next()
         read_seq = itertools.chain( [ first_read ], read_seq )
      pe_mode = first_read.paired_end
   except:
      sys.stderr.write( "Error occured when reading first line of SAM/BAM file.\n" )
      raise
      ...
      for r in read_seq:
         ...
         if not pe_mode:
            ...
            if ( r[0] and r[0].aQual < minaqual ) or ( r[1] and r[1].aQual < minaqual ):
               ...
               write_to_samout( r, "too_low_aQual" )
               continue
            nh = (r[0].optional_field( "NH" ) + r[1].optional_field( "NH" )) / 2
...
def main():
   ...
   optParser.add_option( "--allow_ambiguous", type="choice", dest="allow_ambiguous",
   		choices = ( "yes", "no" ), default = "no",
   		help = "whether to allow mappings to multiple transcripts" )
   		
   optParser.add_option( "--allow_nonunique", type="choice", dest="allow_nonunique",
   		choices = ( "yes", "no" ), default = "no",
   		help = "whether to allow nonunique mapping" )
   		
   optParser.add_option( "--use_NH", type="choice", dest="use_NH",
   		choices = ( "yes", "no" ), default = "no",
   		help = "whether to use NH tag (ie scale counts by number of alignments reported)" )
   ...
   try:
      count_reads_in_features( args[0], args[1], opts.samtype, opts.order, opts.stranded, 
         ...
         opts.samout, opts.allow_ambiguous, opts.allow_nonunique, opts.use_NH )
   except:
